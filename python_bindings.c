...
PyMODINIT_FUNC init_suffix_tree() {
...
#ifdef PY3
    if (PyBytes_AsStringAndSize(PyUnicode_AsUTF8String(string),&s,&n))   return -1; /* rethrow */
    if (PyBytes_AsStringAndSize(PyUnicode_AsUTF8String(terminal),&t,&n)) return -1; /* rethrow */
#else
